...
const login = async (email, password) => {
  ...
  try {
    ...
    if (res.data.status === 'success') {
      (0, _alert.showAlert)('success', 'logged in sucessful');
      window.setTimeout(() => {
        location.assign('/');
      }, 5000);
    }
...
const displayMap = locations => {
  mapboxgl.accessToken = 'pk.eyJ1IjoibmVlbDA0NSIsImEiOiJja2oyYzBkbDYzOWk4MnhzY2N3azVwOW9iIn0.D-hDSIaUkOzRC2KmJKZ26A';
  ...
  locations.forEach(loc => {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML(`<p>Day :${loc.day}: ${loc.description}</p>`).addTo(map); //Extends map bounds to include to current location
...
const bookTour = async tourId => {
  try {
    const stripe = Stripe('pk_test_51I2amPJVKdSGPgoYuy9LZbRLoMUubioFDHkK4VJ90Mt24P0dqtV3vfniM07NPpbgPSWjADbZ54Zvu1uFS3WqF24300x2A5ckP6'); // 1)Get the session from API

    const session = await (0, _axios.default)(`http://127.0.0.1:3000/api/v1/bookings/checkout-session/${tourId}`);
    console.log(session); //   2) Create a checkout form and charge the userDataForm
...
if (bookBtn) {
  bookBtn.addEventListener('click', e => {
    ...
    const {
      tourId
    } = e.target.dataset; //tour-id => tourId(js ka jadu) and destructuring
